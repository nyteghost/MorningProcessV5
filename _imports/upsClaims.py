...
suffix2 = r"\Southeastern Computer Associates, LLC\GCA Deployment - Documents\Database\Daily Data Sets\Pre-Database\Report.xls"
...
for col in report_df.columns:
    print(col)
# paList = report_df['Paid Amount'].tolist()
# print(paList)
print('########################################################################')
...
appended_df = appended_df.drop(columns=['Check/Electronic Deposit Number', 'Check/Electronic Deposit Date', 'UPS Account #','Filed Claim Amount'])
...
# print(appended_df)
appended_df.columns = appended_df.columns.str.lower()
importlog.info(appended_df)
# for col in appended_df.columns:
#     print(col)
# exit()
connect = dbConnect("shipping")
connect.df_to_sql(appended_df, 'dbo_dailyclaimssummary')
connect.call('dbo_uspupdateclaimssummarydata')
